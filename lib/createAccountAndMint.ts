...
const USDC_MINT_AMOUNT = BigInt(1000);
const ETH_MINT_AMOUNT = BigInt(1);
...
async function importFaucetAccount(
  ...
  if (!faucet) {
    ...
    if (!faucet) {
      throw new Error(`${faucetName} faucet not found after import: ${faucetId}`);
    }
...
async function consumeMintedNotes(
  ...
  if (mintedNoteIds.length > 0) {
    ...
    const consumeTxResult = await client.newTransaction(account.id(), consumeTxRequest);
...
export async function createAccountAndMint(): Promise<CreateAccountResult> {
  ...
  try {
    ...
    const { WebClient, AccountStorageMode, AccountId, NoteType, TransactionProver } = MidenSDK;

    // Initialize Miden client and delegated prover
    const client = await WebClient.createClient(ENV_CONFIG.MIDEN_NODE_ENDPOINT);
    const prover = TransactionProver.newRemoteProver(ENV_CONFIG.TX_PROVER_ENDPOINT);
    ...
    const newAccount = await client.newWallet(AccountStorageMode.public(), true);
    ...
    console.log(`Waiting ${TRANSACTION_CONFIRMATION_DELAY / 1000} seconds for transaction confirmation...`);
