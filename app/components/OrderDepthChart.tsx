        ...
        <div className="flex-1 overflow-y-auto min-h-0">
            ...
            .map((ask, index) => (
              <div key={`ask-${index}`} className="relative flex-shrink-0">
                ...
                <div className="relative grid grid-cols-12 gap-2 text-xs py-0.5 px-2">
                  ...
                  <span className="col-span-4 text-white text-center">
                    {ask.amount.toFixed(4)}
                  </span>
                  <span className="col-span-4 text-gray-300 text-right">
                    {ask.total.toFixed(4)}
                  </span>
                </div>
              </div>
          ...
          {bids.map((bid, index) => (
            <div key={`bid-${index}`} className="relative flex-shrink-0">
              ...
              />
              <div className="relative grid grid-cols-12 gap-2 text-xs py-0.5 px-2">
                <span className="col-span-4 text-green-400">
        ...
        <div className="border-t border-gray-700 pt-4 space-y-2 text-sm flex-shrink-0 mt-2">
          <div className="flex justify-between text-gray-400">
            ...
            <span className="text-green-400">
              {bids.reduce((sum, bid) => sum + bid.amount, 0).toFixed(4)}{" "}
              {baseAsset}
            </span>
          </div>
          <div className="flex justify-between text-gray-400">
            ...
            <span className="text-red-400">
              {asks.reduce((sum, ask) => sum + ask.amount, 0).toFixed(4)}{" "}
              {baseAsset}
            </span>
          </div>
        </div>
